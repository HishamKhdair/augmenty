...
def token_swap_augmenter(
    ...
    for i in range(n_tok):
        ...
        if random.random() < level:
            ...
            if respect_eos and (
                (
                    example.y[i].is_sent_end is True
                    or example.y[min_i].is_sent_end is True
                )
            ):
                continue
            ...
            if example.y[min_i].ent_iob in {0, 2} and example.y[i].ent_iob in {
                ...
                example.y[min_i].ent_iob == 3
                and example.y[i].ent_type == 0
                or example.y[i].ent_iob == 3
            ...
            for k in tok_anno:
                if k in ["SENT_START", "SPACY"]:
                    continue
                if k == "HEAD":
                    if not example.y.has_annotation("HEAD"):
                        continue
            ...
            if respect_ents and swap_ents is True:
